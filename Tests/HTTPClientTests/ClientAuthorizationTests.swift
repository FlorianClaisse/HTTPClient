...
final class HTTPClientAuthorizationTests: XCTestCase {
    ...
    func testFailingWillSendRequestDoesntTriggerRetry() async throws {
        ...
        class MockFailingDelegate: HTTPClientDelegate {
...
private final class MockAuthorizingDelegate: HTTPClientDelegate {
    var token: Token!
    ...
    func client(_ client: HTTPClient, shouldRetry task: URLSessionTask, error: Error, attempts: Int) async throws -> Bool {
        if case .unacceptableStatusCode(let statusCode) = error as? HTTPClientError,
