...
For smaller apps or one-off requests, using ``HTTPClient`` directly without creating an API definition can be acceptable. For example:

```swift
let request = Request(path: "/user/emails", method: .post, body: ["florian@me.com"])
...
> tip: This API is visually appealing, but it can be a bit tedious to write. With [CreateAPI](https://github.com/kean/CreateAPI), you can take your backend OpenAPI spec, and generate all of the response entities and even requests for ``HTTPClient``.
