...
```swift
var request = Request(path: "/user/emails", method: .post)
...
While you can use ``send(_:delegate:configure:)-ep38`` to send data to the server, ``HTTPClient`` also provides a convenience ``upload(for:fromFile:delegate:configure:)-4jqcp`` method for uploading data from a file.
...
``HTTPClient`` provides elegant high-level APIs, but also gives you _complete_ access to the underlying `URLSession` APIs. You can, as shown earlier, change the session configuration, but it doesn't stop there. You can also provide a custom `URLSessionDelegate` and implement only the methods you need and ``HTTPClient`` will handle the rest.
